...
contextBridge.exposeInMainWorld('samudraAPI', {
  ...
  getLegalDocuments: () => ipcRenderer.invoke('db:legal-documents'),
  getLegalDocumentById: (id) => ipcRenderer.invoke('db:legal-document', id),
  resolveLegalBasis: (query, limit) => ipcRenderer.invoke('db:resolve-legal-basis', query, limit),
  importDocument: (payload) => ipcRenderer.invoke('db:import-document', payload),
